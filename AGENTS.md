...
The frontend has its own dedicated guidance at **`ObsTool/ObsToolClient/AGENTS.md`** — read it before doing any frontend work. It covers the MUI v6 / tss-react styling shim, dev-server port rules, and testing setup. Do not duplicate that content here.

## Important rules

- **Never commit to git.** This applies to both backend and frontend changes. (Repeated from the frontend AGENTS.md so it isn't missed when working server-side.)
...
npm run dev        # Vite dev server (see ObsToolClient/AGENTS.md for port rules)
